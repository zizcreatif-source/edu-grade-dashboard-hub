...
export function CourseDetails({ coursId }: CourseDetailsProps) {
  const { cours, etablissements, etudiants, notes, evaluations } = useData();
  ...
  const heuresEffectuees = Math.round((currentCours.quantumHoraire * currentCours.progression) / 100);
    ...
    <div className="space-y-6">
      ...
      {coursEvaluations.length > 0 && (
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {coursEvaluations.slice(0, 5).map((evaluation) => (
